"""Class definitions for the bundled portfolio model artifacts.

Lives in its own module (rather than inline in `generate.py`) so the
pickled instances record a stable, importable path on disk
(`_classes.MortgageRateTSModel` rather than `__main__.MortgageRateTSModel`).
The sandboxed runner adds the artifact's directory to `sys.path` before
unpickling, so `import _classes` resolves cleanly.

If you bring your own `.pkl` with custom classes, the same trick works:
ship a sibling `.py` file alongside the artifact and the sandbox finds it.
"""
